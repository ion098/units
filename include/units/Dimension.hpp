...
struct Dimensions {
    ...
    Ratio luminosity{};
    Ratio moles{};
    friend constexpr Dimensions operator+(Dimensions dim1, Dimensions dim2) {
        dim1.mass = dim1.mass + dim2.mass;
        dim1.length = dim1.length + dim2.length;
        dim1.time = dim1.time + dim2.time;
        dim1.current = dim1.current + dim2.current;
        dim1.angle = dim1.angle + dim2.angle;
        dim1.temperature = dim1.temperature + dim2.temperature;
        dim1.luminosity = dim1.luminosity + dim2.luminosity;
        dim1.moles = dim1.moles + dim2.moles;
        return dim1;
    }
    friend constexpr Dimensions operator-(Dimensions dim1, Dimensions dim2) {
        dim1.mass = dim1.mass - dim2.mass;
    ...
    friend constexpr Dimensions operator*(Dimensions dim, Ratio scale) {
        dim.mass = dim.mass * scale;
        dim.length = dim.length * scale;
        dim.time = dim.time * scale;
        dim.current = dim.current * scale;
        dim.angle = dim.angle * scale;
        dim.temperature = dim.temperature * scale;
        dim.luminosity = dim.luminosity * scale;
        dim.moles = dim.moles * scale;
        return dim;
    }
    friend constexpr Dimensions operator*(Ratio scale, Dimensions dim) {
        return dim * scale;
    }
    friend constexpr Dimensions operator/(Dimensions dim, Ratio scale) {
        dim.mass = dim.mass / scale;
